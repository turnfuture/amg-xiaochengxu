...
Page({
  ...
  clickTab: function (e) {
    ...
    if (_this.data.currentTab == '3') { //可提现
      _this.moneyInfo(3, '加载中')
    } else if (_this.data.currentTab == '4') { //待结算
      _this.moneyInfo(4, '加载中')
    }
  },
  ...
  getUserMsg: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          _this.setData({
            todayPrice: res.data.data.todayPrice,
            yesdayPrice: res.data.data.yesdayPrice
          })
        } else {
          wx.showToast({
            title: res.data.msg,
